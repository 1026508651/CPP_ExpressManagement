    ...
    ui(new Ui::pkgManRecv)
{
    ...
    if(Packages.searchByPackgeMan(currentUser->getName(*currentUser),*currentUser)==SUCCESS){
        ...
        for(i=0;i<Packages.checkSize;i++){
             ...
             if(Packages.list[Packages.checkOutcome[i]].status!=PKG_WAITSEND){
...
void pkgManRecv::on_Get_clicked()
{
    ...
    for(i=0;i<count1;i++){
        if(model_send->item(save_index[i],7)->checkState()==Qt::Checked){
            currentUser->packgeUsrSendPkg(Packages.list[Packages.checkOutcome[save_index[i]]]);
        }
    }
    stringstream ss;
    string s;
